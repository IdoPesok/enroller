...
export default function ScheduleChangePreview({
  ...
  const [newSections, setNewSections] = useState<null | EnrolledWithSection[]>(null)
  ...
  const oldSection = trpc.sections.withId.useQuery(
    { id: oldSectionId },
  )

  const newSection = trpc.sections.withId.useQuery(
    { id: newSectionId },
  )
  ...
  const populateNewSections = (data: EnrolledWithSection[]) => {
    if (data && userId) {
      ...
      if (isCreating && newSection.data) {
        ...
        const index = temp.findIndex((enrolled) => enrolled.SectionId === oldSectionId)
        if (index !== -1) {
          temp.splice(index, 1)
        }
  ...
  const enrolledSections = trpc.home.userSections.useQuery(
    { types: [Enrolled_Type.Enrolled, Enrolled_Type.Waitlist, Enrolled_Type.ShoppingCart] },
    {
      onSuccess(data) {
    ...
    <div className="flex flex-col mt-5">
      <div className="flex justify-between items-center">
        <p className="my-0 flex gap-2">
          <AlertTriangle />
          {
            isCreating ? "Please confirm: your new schedule if you add " :
            isDropping ? "Please confirm: your new schedule if you drop " :
            isSwapping ? "Please confirm: your new schedule if you swap " : ""
          }
          {
            isCreating ? `${newSection.data?.Courses.Code}(${newSection.data?.SectionId})` :
            isDropping ? `${oldSection.data?.Courses.Code}(${oldSection.data?.SectionId})` :
            isSwapping ? `${oldSection.data?.Courses.Code}(${oldSection.data?.SectionId}) with ${newSection.data?.Courses.Code}(${newSection.data?.SectionId})` : ""
          }
        </p>
        <Tabs dir="ltr" value={viewType} className="ml-4" defaultValue={"before"}>
          <TabsList>
        ...
        <WeekCalendar
          ...
          sections={
            (enrolledSections.data && newSections) ? (
              viewType === "before" ? enrolledSections.data : newSections
            ): []
          }
      ...
      <div className="flex justify-end gap-2 w-full mt-10">
        ...
        <Button size="lg" className="bg-emerald-500 text-white hover:bg-emerald-600" onClick={onConfirm}>
          Confirm
        </Button>
      </div>
    </div>
